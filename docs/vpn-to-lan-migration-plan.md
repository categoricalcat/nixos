...
2. **Subnet Router Setup**: You will need to configure Netbird (or Tailscale) to advertise the LAN route (e.g., `192.168.1.0/24`) and ensure your client devices are configured to route LAN traffic through the VPN.
...
2. Disconnect a client from the local LAN (e.g., connect to a cellular hotspot), enable Netbird, and attempt to access `*.fufu.land` and internal services via their LAN IP addresses.
3. Verify that `lan-mouse`, distributed Nix builds, and `llama-swap` can communicate seamlessly across the LAN interfaces.
