...
- One portable home module set feeds two adapters: the NixOS adapter ([`modules/home-manager.nix`](../../../modules/home-manager.nix) `mkHome`, behavior unchanged) and a new standalone adapter (`homeConfigurations."yi@generic"`).
...
flowchart TD
  subgraph nixosrepo [nixos repo - single source of truth]
    ...
    hmGeneric["homeConfigurations.yi@generic (standalone adapter)"]
  end
  hm --> mkHome
  hm --> hmGeneric
...
## 2. Workstream B - add the standalone `homeConfigurations."yi@generic"`
...
```nix
flake.homeConfigurations."yi@generic" = inputs.home-manager.lib.homeManagerConfiguration {
...
- Standalone adapter: `homeConfigurations."yi@generic"` imports the same profile and sets the same options inline.
...
- Standalone: `nix build .#homeConfigurations."yi@generic".activationPackage` (and `home-manager switch --flake .#yi@generic` on a non-NixOS box).
- Per host: `nixos-rebuild build --flake .#<host>` (no more `--override-input thefiles`).
- CI guard: add a step building the generic profile so an accidental `osConfig`/secret read can never silently break the portable path.
...
home-manager switch --flake github:categoricalcat/nixos#yi@generic   # any distro
```

Develop (edit + test before push): clone `nixos` directly and build with `--flake .#<host>` / `--flake .#yi@generic`. No submodule init, no `--override-input`.
...
3. Add `homeConfigurations."yi@generic"` + the CI eval guard.
...
- Risks:
  - Portability discipline must hold across every module in `home/` - one `osConfig`/secret read breaks `#yi@generic`. The CI build of the generic profile is the guard.
