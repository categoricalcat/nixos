...
2. **Socket & Loopback Binding Isolation**: Bind internal daemons to `127.0.0.1` or UNIX domain sockets (`/run/...`) where external network packets cannot reach them.
3. **Cryptographic Identity & Transport Security**: Authenticate and encrypt inter-host traffic via Mutual TLS (mTLS) or WireGuard Cryptokey Routing instead of brittle IP allowlisting.
4. **Application-Layer Authentication**: Enforce native credentials/passwords on sensitive services like Valkey/Redis.
5. **Systemd eBPF Cgroup Sandboxing**: Use declarative `IPAddressAllow=` / `IPAddressDeny=` at the systemd unit level for granular isolation without manual nftables scripting.
6. **Isolated Container Networks**: Use Podman native `internal = true` bridge networks for container isolation.
...
2. **The "Whack-a-Mole" Drop Chains**: Because `eno1` is trusted, every service listening on `0.0.0.0` or `10.42.0.2` is automatically exposed to any device on the LAN. To mitigate this:
...
3. **Fragility of L3/L4 Packet Filtering**:
...
```mermaid
flowchart TD
    P1[Phase 1: Default-Deny & Remove trustedInterfaces] --> P2[Phase 2: Localhost & Unix Socket Binding]
    P2 --> P3[Phase 3: Valkey Auth / Unix Socket Hardening]
    P3 --> P4[Phase 4: Inter-Host Ingress & mTLS]
    P4 --> P5[Phase 5: Container Bridge Isolation & Clean Up nftables]
    P5 --> DONE[Fully Hardened Zero-Trust Architecture]
```
...
2. For same-host monitoring exporters, ensure all Prometheus exporters connect via `/run/.../*.sock` or `127.0.0.1`.
...
2. Configure Unbound to authenticate against Valkey, or switch to Unix socket binding on `yifuwuqi` if caching is made host-local.

### Phase 4: Inter-Host mTLS / WireGuard Proxying

1. Implement the 3-tier mTLS design from `mtls-architecture-plan.md`.
2. Terminate backend TLS on `yifuwuqi` requiring client certificates from `yirukou`.
3. Inter-host traffic is now cryptographically secured against spoofing, eavesdropping, and perimeter bypass.
...
2. Remove the `container-isolation` custom nftables table in `firewall.nix`.
...
2. **Step 2: Remove `eno1` from `trustedInterfaces` & test**: Verify default-deny blocks backend ports cleanly.
3. **Step 3: Remove custom nftables tables (`backend-ui-guard`, `valkey-guard`, `container-isolation`)**: Verify clean, standard NixOS firewall configuration.
4. **Step 4: Deploy Phase 2 of mTLS Architecture Plan**: Enable encrypted, authenticated inter-host proxying between `yirukou` and `yifuwuqi`.
...
2. **Ingress Architecture**: For inter-host proxying from `yirukou` to `yifuwuqi`, do you prefer the **mTLS Nginx Upstream** approach (Tier 2 in `mtls-architecture-plan.md`) or a dedicated **WireGuard point-to-point peer** between the two hosts?
3. **Systemd eBPF vs Port Binding**: For local services, are you happy standardizing on `127.0.0.1` binding and UNIX sockets, using systemd `IPAddressAllow` only for edge cases?
