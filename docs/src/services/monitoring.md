...
- The Valkey exporter (nixpkgs `services.prometheus.exporters.redis`, port 9121) runs only on the central host and reaches the shared valkey over its unix socket `/run/redis/redis.sock` (same-host, so it bypasses the `valkey-guard` nftables table). The custom `valkey.json` dashboard shows the DNS cache (db0): entry count, keys with expirations, hit ratio, memory vs the 1 GiB max, and eviction/expiry rates.
