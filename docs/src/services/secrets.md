...
The read-only `ai` account (see [ai-ssh](ai-ssh.md)) mirrors the `yi` mesh-key
pattern — each mesh host's `ai` key is authorized on every host — but its keys
are **not** SOPS age recipients. The ai gate needs no secrets, so the ai keys
